"""
Resamples a SOLUS-derived soil type map (.tif) to match the extent, grid
configuration, and projection of a watershed DEM .asc file, producing the
soil type .asc grid used as a VELMA input. Variant of solus_to_velma_grid.py
that uses nearest-neighbor resampling appropriate for categorical soil IDs.
"""
...
SOLUS_data = {'Skagit_9soiltypes_new': 'path/to/SOLUS/soil_types_map.tif'
              }

# Define the watershed DEM .asc file
watershed_dem_asc = ('path/to/VELMA_Watersheds/Skagit/Data_Inputs30m/m_1_DEM/Skagit_DredgeMask_EEX.asc')

# Define desired output folder of clipped SOLUS data
out_path = 'path/to/VELMA_Watersheds/Skagit/Data_Inputs30m/m_6_Soils/'


# Resample the SOLUS 100m data to match the cell resolution and location of the DEM .asc file
def resample_tif_to_match_asc(tif_file, asc_file, output_asc):
    # IMPORTANT!!
    # The following two lines assume the projection for the .asc and .tif files are consistent across the Puget Sound.
    # Can change these lines if the projection differs.
    asc_crs = CRS.from_epsg(26910)  # NAD83 / UTM Zone 10N
    tif_crs = CRS.from_epsg(26910)  # NAD83 / UTM Zone 10N
...
for var, tif_file in SOLUS_data.items():
    ...
    resample_tif_to_match_asc(tif_file, watershed_dem_asc, output_asc)
