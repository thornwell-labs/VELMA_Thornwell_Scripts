"""
Plots monthly-mean trends of a variable (e.g. runoff) by decade, summed across
all 20 PSIMF watersheds, to show how the seasonal (monthly) pattern shifts
through the century.
"""
...
year_range = list(range(2020, 2030)) + list(range(2090, 2100))
...
decade_variable["Percent Difference"] = (
    (decade_variable[2090] - decade_variable[2020]) / decade_variable[2020])*100
...
plt.xlabel("Month")
plt.title(f"{variable_name} Percent Difference by Month: 2090s vs 2020s")
plt.tight_layout()
plt.savefig(f"{out_folder}/{variable_name}_percent_difference_by_month_2090_vs_2020.png", dpi=300)
plt.show()
